	...
	for row in range(BRICK_ROWS):
		for col in range(BRICK_COLS):
			var position := Vector2(
				start_x + col * (BRICK_SIZE.x + BRICK_GAP),
				66.0 + row * (BRICK_SIZE.y + BRICK_GAP)
			)
			_bricks.append({
...
func _check_paddle_collision() -> void:
	...
	var ball_rect := Rect2(_ball_position - Vector2.ONE * BALL_RADIUS, Vector2.ONE * BALL_RADIUS * 2.0)
...
func _draw_background() -> void:
	...
	for y in range(0, int(BOARD_SIZE.y), 32):
		draw_line(_board_origin + Vector2(0.0, y), _board_origin + Vector2(BOARD_SIZE.x, y), Color(1, 1, 1, 0.035), 1.0)
...
func _draw_paddle() -> void:
	...
	draw_rect(Rect2(rect.position + Vector2(10.0, 4.0), Vector2(rect.size.x - 20.0, 4.0)), Color("5cc8ff"))
...
func _draw_overlay() -> void:
	...
	draw_rect(Rect2(position - Vector2(18.0, 26.0), text_size + Vector2(36.0, 42.0)), Color(0, 0, 0, 0.55))
