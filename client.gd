# res://addons/indigauge/client.gd
...
# Simple “player id” persistence similar idea to Rust helper (native). 14
...
func start_session() -> void:
	...
	# Godot doesn’t expose total RAM portably; you can plug this in if you have it.
	p.memory = ""

	var url := config.api_url("sessions/start") # 15
	var headers := [
...
func _dispatch_from_core(level: String, event_type: String, metadata: Variant, _ctxdict: Dictionary) -> bool:
	# Like Bevy plugin: only queue events after session started. 16
	if _session_token == "":
		return false
	...
	e.context = null # Godot lacks file/line/module in runtime
...
func flush_events() -> int:
	...
	match mode:
		...
		IndigaugeTypes.Mode.LIVE:
			var url := config.api_url("events/batch") # 17
			var headers := [
				"Content-Type: application/json",
...
func send_heartbeat() -> void:
	...
	match mode:
		...
		IndigaugeTypes.Mode.LIVE:
			var url := config.api_url("sessions/heartbeat") # 18
			var headers := [
				"Content-Type: application/json",
				"X-Indigauge-Key: %s" % _session_token,
			]
...
func submit_feedback(message: String, category: String, question: String = "", include_screenshot: bool = false) -> void:
	...
	match mode:
		IndigaugeTypes.Mode.DEV:
			...
			return
		IndigaugeTypes.Mode.LIVE:
			var url := config.api_url("feedback") # 19
			var headers := [
				"Content-Type: application/json",
				"X-Indigauge-Key: %s" % _session_token,
			]
...
func _upload_feedback_screenshot(feedback_id: String) -> void:
	# Matches POST feedback/{id}/screenshot with image/png. 20
