...
static func validate_event_type(s: String) -> Dictionary:
	...
	if dot == 0 or dot == s.length() - 1:
		return {
			"ok": false, "error": "Invalid event type: '.' cannot be the first or last character"
		}
	for ch in s:
		if ch == ".":
			continue
		...
		if not is_letter:
			return {
				"ok": false,
				"error": "Invalid event type: only letters and a single '.' are allowed"
			}
	return {"ok": true}


static func emit(
	level: String, event_type: String, metadata: Variant = null, ctx: Dictionary = {}
) -> bool:
...
static func coarsen_cpu_name(cpu_raw: String) -> String:
	...
	if name.find("apple m1") != -1:
		return "Apple M1"
	if name.find("apple m2") != -1:
		return "Apple M2"
	if name.find("apple m3") != -1:
		return "Apple M3"
	if name.find("intel") != -1:
		if name.find("celeron") != -1:
			return "Intel Celeron"
		if name.find("pentium") != -1:
		...
		return "Intel (Other)"
	if name.find("amd") != -1:
		if name.find("threadripper") != -1:
			return "AMD Ryzen Threadripper"
		if name.find("ryzen") != -1:
			return "AMD Ryzen"
		if name.find("epyc") != -1:
			return "AMD EPYC"
		if name.find("athlon") != -1:
			return "AMD Athlon"
		return "AMD (Other)"
